...
const Navbar = () => {
    ...
    const dettagliMenu = [
        ...
        { id: 8, voceLi: "Regolamento Carriera FC24 ", linkTo: "/regolamento" },
    ];
    ...
    const linksMenu = dettagliMenu.map((voce) => {
        return (
            <div key={voce.id}>
                <li className="hover:text-yellow-600 py-6 text-xl font-bold uppercase">{voce.voceLi}</li>
            </div>
        );
    });
    ...
    return (
        <nav className="w-full h-auto py-3 px-6 fixed flex items-center justify-between z-[1000]">
            ...
            />
            <div className="cursor-pointer flex items-center justify-center rounded-full p-2 ">
                {!openMenu ? (
                    <MdMenu size={36} className="" onClick={handleClick} />
                ) : (
                    <MdClose
                        size={36}
                        className="fill-gray-300 hover:fill-gray-200"
                ...
                <ul
                    ...
                    className={
                        openMenu
                            ? "h-screen w-full absolute top-0 right-0 text-center flex flex-col justify-around items-center bg-black/95 text-gray-300 z-[-1] transition-[0.3s] py-20 px-28"
                            : "h-screen w-full absolute top-0 right-[-100%] text-center flex flex-col justify-around items-center bg-black/95 text-gray-300 z-[-1] transition-[0.3s] py-20 px-28"
                    }
                >
                  {linksMenu}
                </ul>

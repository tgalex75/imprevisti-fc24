...
const RegistroMercato = (props) => {
  const { vociRegistro, deleteListDB, removeVociRegistro, tipoImprevisto } = props;
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ delay: 0.7, duration: 0.7 }}
      className="absolute bottom-1 left-1 top-1 hidden h-[98%] w-[20vw] items-center gap-2 overflow-hidden rounded-lg bg-black/50 text-gray-300 md:flex md:flex-col"
    >
      ...
      <ul className="flex h-full w-full flex-col gap-1 overflow-y-auto px-2 pb-2">
        {vociRegistro.map((el) => (
          ...
          <li
            key={el.id}
            style={el.tipo === tipoImprevisto ? {borderLeft: "3px solid var(--clr-ter)"} : {}}
